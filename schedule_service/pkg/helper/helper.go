...
func GenerateLogin(prefix string, table string, db *pgxpool.Pool) string {
	var logingen string
	query := `SELECT login FROM `
	query += table
	query += " ORDER BY created_at DESC LIMIT 1"
	var scanlogin sql.NullString
	_ = db.QueryRow(context.Background(), query).Scan(
		&scanlogin,
	)

	var lastnumber string
	fmt.Println(len(scanlogin.String))
	if len(scanlogin.String) > 0 {
		for _, v := range scanlogin.String {
			if v > 47 && v < 58 {
				lastnumber += string(v)
			}
		}
		...
		lastnumberint++
		lastnumber = strconv.Itoa(lastnumberint)
		logingen = LoginGenerate(prefix, lastnumber)
	} else {
		logingen = LoginGenerate(prefix, "1")
	}
...
func LoginGenerate(prefix string, number string) string {
	var login string
	login += prefix
	digit := 5
	for i := 0; i < digit-len(number); i++ {
		login += "0"
	}
	login += number
	return login
...
func GetGroupUniqueId(n int) string {
	t := "0000000"
